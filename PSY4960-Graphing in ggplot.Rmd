...
```{r}
myDiamonds<- subset(diamonds, diamonds$carat>= 1)

summarySE(myDiamonds, measurevar = "price", groupvars = "clarity")

diamondsGraph<- summarySE(myDiamonds, measurevar = "price", groupvars = "clarity")
```
...
```{r}
ggplot(diamondsGraph)
ggplot(diamondsGraph, aes(price)) +geom_bar()
```
...
```{r}
ggplot(myDiamonds, aes(price))+geom_histogram()
```
...
```{r}
ggplot(myDiamonds, aes(price))+ geom_histogram()+ theme_classic()
```
...
```{r}
mygraph<-ggplot(myDiamonds, aes(price))+ geom_histogram(fill = "pink", color="yellow")+ theme_bw()
mygraph

#ggplot(myDiamonds, aes(price), fill = "blue", color="yellow")+ geom_histogram()+ theme_bw()
#works for some graphs, but not this one! 
```
...
```{r}
ggplot(myDiamonds, aes(carat))+ geom_histogram()
```
2. Include a theme to change the appearance of your graph.
```{r}
ggplot(myDiamonds, aes(carat))+ geom_histogram()+ theme_dark()

```

3. Create a histogram with orange bars and purple lines. 
```{r}
ggplot(myDiamonds, aes(carat))+ geom_histogram(fill= "orange", color= "purple")
```
...
```{r}
ggplot(myDiamonds, aes(clarity))+ geom_bar()+ theme_bw()
```
...
```{r}
ggplot(myDiamonds, aes(clarity))+ geom_bar()+ xlab(label = "\n Clarity of Diamonds") + ylab(label = "Frequency \n")
```
Note - \n- starts a new line 
...
```{r}
ggplot(diamondsGraph, aes(clarity, price))+ geom_bar(stat="identity", color= "blue", fill="lightblue")
```
...
```{r}
ggplot(diamondsGraph, aes(clarity, price))+ geom_bar(stat="identity", color= "blue", fill="lightblue",alpha = .7, position= position) + geom_errorbar(aes(ymin = price-se, ymax= price+se, width= .5 ))
```
Things I included:
...
**adding lines**
```{r}
ggplot(diamondsGraph, aes(clarity, price))+ geom_bar(stat="identity", color= "blue", fill="lightblue",alpha = .7, position= position_dodge()) + geom_errorbar(aes(ymin = price-se, ymax= price+se, width= .5 ))+ geom_hline(yintercept = 5000, color="purple", size=1, linetype="dashed")
...
```{r}
ggplot(myDiamonds, aes(cut))+geom_bar(fill= "purple", color= "deeppink")
```

2. Change the x and y labels to something that is more informative. 
```{r}
ggplot(myDiamonds, aes(cut))+geom_bar(fill= "purple", color= "deeppink")+ xlab("Cut of Diamond") + ylab("No. of Diamonds")

```

###Adding a third variable to a bar graph: 
```{r}
ggplot(myDiamonds, aes(price, fill= cut)) + geom_histogram()
```

**Scatter plot**
We can make scatter plots using the argument geom_point(). We'll make a graph of carat by price. 
```{r}
ggplot(myDiamonds, aes(carat, price, shape= clarity))+ geom_point(color="purple")
```
...
```{r}
ggplot(diamonds, aes(x,y))+geom_point(color="pink", shape=10)
...
```{r}
ggplot(myDiamonds, aes(carat, price)) +geom_point()+ geom_smooth(method= "lm", se = TRUE)
...
```{r}
data(mtcars)
ggplot(mtcars, aes(mpg, hp)) + geom_point() + geom_smooth(method="lm", se=FALSE, color= "orange", linetype="dashed")+theme_classic()
```

**Changing scatterplots**
you can change the outline of a dot using "color" 
```{r}
ggplot(mtcars, aes(mpg, hp))+ geom_point(color= "purple")
```

you can also change the shape and the size of the points by adding shape and size arguments to geom_point. 
```{r}
ggplot(mtcars, aes(mpg, hp))+ geom_point(color= "purple", shape= 21)
```

####Practice: 
1. Create a scatter plot of the table and carat variables. 
```{r}
ggplot(myDiamonds, aes(table, carat))+ geom_point()
```
2. Change the color of the points to your favorite color. 
```{r}
ggplot(myDiamonds, aes(table, carat))+ geom_point(color="orange")
```
3. Change the shape of the points. 
```{r}
ggplot(myDiamonds, aes(table, carat))+ geom_point(color="orange", shape= 13)
```
4. Add a line of best fit to your scatter plot. 
```{r}
ggplot(myDiamonds, aes(table, carat))+ geom_point(color="orange", shape= 13)+geom_smooth(method="lm", color="black", size= 1)
```
...
```{r}
ggplot(myDiamonds, aes(price, carat))+ geom_line() #not very helpful

ggplot(mtcars, aes(gear, mpg))+ geom_line() #not very helpful
"making it more useful"
data(ChickWeight)

ggplot(ChickWeight, aes(Time, weight,group =Chick, color=Chick))+ geom_point()+ geom_line()+theme_classic()
```
...
```{r}
ggplot(myDiamonds, aes(price))+ geom_boxplot()
ggplot(myDiamonds, aes(price, color))+ geom_boxplot()
ggplot(myDiamonds, aes(color, price))+ geom_boxplot()
```
####Practice: 
1. Create a boxplot for carat.
```{r}
ggplot(myDiamonds, aes(carat))+ geom_boxplot()
```

2. Create boxplots of carat for each level of cut.
```{r}
ggplot(myDiamonds, aes(cut,carat))+ geom_boxplot()

```

3. **Bonus** play around with fill and color. What changes about your boxplots? 
```{r}
ggplot(myDiamonds, aes(cut,carat))+ geom_boxplot(fill= "cyan", color="green")+ theme_classic()
...
For bar graphs: 
```{r}
ggplot(myDiamonds, aes(cut, fill= color))+geom_bar()+theme_classic()

ggplot(myDiamonds, aes(cut, fill= color))+geom_bar(position=position_dodge())+theme_classic()
```

For scatter plots: 
```{r}
ggplot(myDiamonds, aes(price, carat, color= cut))+ geom_point()+theme_classic()

ggplot(mtcars, aes(mpg, hp, color= as.factor(gear)))+ geom_point()+theme_classic()
```
...
```{r}
ggplot(myDiamonds, aes(price, carat)) + geom_point(color = "#66FFCC")+ theme_classic()

ggplot(myDiamonds, aes(price, carat, color= cut))+geom_point()+ theme_classic()+ scale_color_manual(values= c("#CC6699", "#FFCC66", "#FF9933", "#660099", "#FFCCFF"))
```
R ColorBrewer
```{r}
ggplot(myDiamonds, aes(price, carat, color= cut))+geom_point()+ theme_classic()+ scale_color_brewer(palette = "Purples")
```

```{r}
ggplot(myDiamonds, aes(price, carat, color= cut))+geom_point()+ theme_classic()+ scale_color_viridis(discrete=TRUE, option = "D")
```
...
```{r}
ggplot(mtcars, aes(cyl, mpg, color=as.factor(am))) + geom_point() + scale_color_brewer(palette= "GnBu",) + theme_classic()
```
...
```{r}
ggplot(mtcars, aes(mpg, hp, color=as.factor(gear))) + geom_point()+ theme_classic()+ scale_color_viridis(discrete=TRUE,option= "B")
